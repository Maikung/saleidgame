...
export default function AuthTester() {
  ...
  const checkProfile = async () => {
    ...
    try { const res = await fetch(`${apiBase}/api/auth/me`, { headers: { Authorization: `Bearer ${token}` } }); const data = await readApiResponse(res); if (!data.user) throw new Error("API response is missing user data"); setResult({ type: "success", message: "GET /api/auth/me สำเร็จ", user: data.user }); } catch (error) { setResult({ type: "error", message: error.message }); }
  };
